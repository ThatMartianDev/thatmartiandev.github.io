...
function scrollHandler(index) {
  ...
  sections.forEach((section, sectionIndex) => {
    if (sectionIndex === currentIndex) {
      ...
      section.querySelectorAll("[tabindex]").forEach((el) => (el.tabIndex = 0));
      links[currentIndex].classList.add("active");
    } else {
      section.classList.remove("active");
      section.querySelectorAll("[tabindex]").forEach((el) => (el.tabIndex = -1));
...
// Add scroll event listener to container with throttle
mainEle.addEventListener("wheel",
  throttle((e) => {
    e.preventDefault();
